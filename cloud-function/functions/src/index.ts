...
export const forecast = functions.https.onRequest(async (request, response) => {
  ...
  try {
    const {data: oneCallData} = await axios.get<OneCall>(`${apiUrl}/onecall?lat=${lat}&lon=${lon}&exclude=minutely,alerts&appid=${apiKey}&units=metric`);
    const {data: geoData} = await axios.get<Geolocation[]>(`${geoUrl}/reverse?lat=${lat}&lon=${lon}&limit=5&appid=${apiKey}`);
