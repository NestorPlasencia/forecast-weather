...
export const solve = (A: number[][], ks: number[]) => {
  const m = A.length;
  let h = 0;
  let k = 0;
  while (h < m && k <= m) {
...
export const zerosMat = (r: number, c: number): number[][] => {
  const A = [];
  for (let i = 0; i < r; i++) A.push(new Array(c).fill(0));
  return A;
};

export const swapRows = (m: number[][], k: number, l: number) => {
  let p = m[k];
  m[k] = m[l];
  m[l] = p;
};
